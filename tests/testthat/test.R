
context("parts");

test_that("all parts are lowercase (except some)", {
  lower <- setdiff(names(malign_parts), "frowny");
  for (p in lower) {
    pp <- setdiff(malign_parts[[p]], "R package");
    expect_true(all(is_all_lowercase(pp)), info=p);
  }
})

context("templating");

test_that("template without malign word", {
  str <- "This is just a random string";
  expect_equal(malign(str), str);
  expect_equal(malign(""), "");
})

test_that("corner cases are OK", {
  malign_check("", "");
  malign_check("x", "x");
  malign_check("${adjective}", "^(?<adjective>.*)$");
})

test_that("template with a single part", {
  malign_check(
    "This is ${adjective}.",
    "^This is (?<adjective1>.*)\\.$"
  );
  malign_check(
    "This is ${adverb}.",
    "^This is (?<adverb1>.*)\\.$"
  );
  malign_check(
    "This is ${adverb_manner}.",
    "This is (?<adverb_manner1>.*)\\.$"
  );
  malign_check(
    "This was ${created}.",
    "^This was (?<created1>.*)\\.$"
  );
  malign_check(
    "This was ${creating}.",
    "^This was (?<creating1>.*)\\.$"
  );
  malign_check(
    "This was ${exclamation}.",
    "^This was (?<exclamation1>.*)\\.$"
  );
  malign_check(
    "This is a terrible ${rpackage}!",
    "^This is a terrible (?<rpackage1>.*)!$"
  );
  malign_check(
    "That was certainly a ${result}!",
    "^That was certainly a (?<result1>.*)!$"
  );
  malign_check(
    "${frowny}!",
    "^(?<frowny1>.*)!$"
  );
})

test_that("templates with multiple parts of the same type", {
  malign_check("This is ${adjective} and ${adjective}.",
               "^This is (?<adjective1>.*) and (?<adjective2>.*)\\.$");
})

test_that("different part types in the same template", {
  malign_check(
    "You did this ${adverb_manner} and it got ${adjective}!",
    "^You did this (?<adverb_manner>.*) and it got (?<adjective>.*)!$"
  );
})

context("Capilatization");

test_that("templates are case insensitive", {
  malign_check("${AdjeCtiVe}", "^(?<adjective>.*)$");
  malign_check(
    "This is ${adjeCtive}.",
    "^This is (?<adjective1>.*)\\.$"
  );
  malign_check(
    "This is ${Adverb}.",
    "^This is (?<adverb1>.*)\\.$"
  );
  malign_check(
    "This is ${Adverb_Manner}.",
    "This is (?<adverb_manner1>.*)\\.$"
  );
  malign_check(
    "This was ${CREATED}.",
    "^This was (?<created1>.*)\\.$"
  );
  malign_check(
    "This was ${creatING}.",
    "^This was (?<creating1>.*)\\.$"
  );
  malign_check(
    "This was ${EXClamation}.",
    "^This was (?<exclamation1>.*)\\.$"
  );
  malign_check(
    "This is a terrible ${rpACKage}!",
    "^This is a terrible (?<rpackage1>.*)!$"
  );
  malign_check(
    "That was certainly a ${reSulT}!",
    "^That was certainly a (?<result1>.*)!$"
  );
  malign_check("This is ${Adjective} and ${Adjective}.",
               "^This is (?<adjective1>.*) and (?<adjective2>.*)\\.$");
  malign_check(
    "You did this ${adVERB_manner} and it got ${Adjective}!",
    "^You did this (?<adverb_manner>.*) and it got (?<adjective>.*)!$"
  );
})

test_that("first letter is capitalized", {
  for (i in 1:13) {
    mal <- malign("${Adjective}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Adverb}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Adverb_manner}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Created}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Creating}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Exclamation}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Rpackage}");
    expect_true(is_capitalized(mal));
    mal <- malign("${Result}");
    expect_true(is_capitalized(mal));
  }
})

test_that("whole word is upper case", {
  for (i in 1:13) {
    mal <- malign("${ADJECTIVE}");
    expect_true(is_all_uppercase(mal), info=mal);
    mal <- malign("${ADVERB}");
    expect_true(is_all_uppercase(mal));
    mal <- malign("${ADVERB_MANNER}");
    expect_true(is_all_uppercase(mal));
    mal <- malign("${CREATED}");
    expect_true(is_all_uppercase(mal));
    mal <- malign("${CREATING}");
    expect_true(is_all_uppercase(mal));
    mal <- malign("${EXCLAMATION}");
    expect_true(is_all_uppercase(mal));
    mal <- malign("${RPACKAGE}");
    expect_true(is_all_uppercase(mal));
    mal <- malign("${RESULT}");
    expect_true(is_all_uppercase(mal));
  }
})
